  ...
  useEffect(() => {
    const fetchMediumPriorityTasks = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          toast(language === "en" ? "Failed to fetch high priority tasks" : "Yüksek öncelikli görevler alınamadı", {
    ...
    <section className="min-h-screen ml-0 md:ml-64 mt-20 flex flex-col items-center px-8">
      <div className="w-full max-w-5xl">
        <div className="text-4xl font-bold text-blue-500">
          {language === "en" ? "High Priority Tasks" : "Yüksek Öncelikli Görevler"}
        </div>
      </div>
      ...
      {loading ? (
        ...
        tasks.length === 0 ? (
          <div className="text-2xl mt-8">{language === "en" ? "No high priority tasks" : "Yüksek öncelikli görev yok"}</div>
        ) : (
