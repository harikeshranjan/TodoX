...
import { Home, Calendar, Flag, ChevronDown, ChevronRight, Plus, LogOut, Settings, Star, CalendarRange } from 'lucide-react';
...
export default function Sidebar() {
  ...
  useHotkeys('shift+k', () => toggleSidebar());
  ...
  }, [isDesktop, setIsSidebarOpen, pathname]);
  ...
  useEffect(() => {
    const checkLoggedIn = async () => {
      try {
        console.log("API URL:", process.env.NEXT_PUBLIC_API_URL); // Debugging step
        ...
        if (response.ok) {
          ...
          router.replace('/login'); // No need for setTimeout
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAllDistinctTags = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          toast(language === 'en' ? 'Error' : 'Hata', {
            description: language === 'en' ? 'Failed to fetch tags' : 'Etiketler getirilemedi',
            duration: 5000,
          });
        }

        const data = await response.json();
        const distinctTags = [...new Set(data.flatMap((doc: any) => doc.tags || []))];
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar">
            ...
            <Button variant="ghost" className="w-full flex justify-between items-center mb-1">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {language === 'en' ? 'Priorities' : 'Öncelikler'}
                </span>
              </div>
        ...
        <div className="space-y-1 py-2">
          ...
          <LinkButton href="/overdue" isActiveLink={isActiveLink}>
            ...
            <span>
              {language === 'en' ? 'Overdue' : 'Gecikenler'}
            </span>
          </LinkButton>
        </div>
      </div>
      ...
      <div className="mt-auto space-y-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="w-full gap-1 border">
              <Settings size={16} />
              <span>
        ...
        <DropdownMenu>
            ...
            <Button variant="ghost" className="w-full h-12 gap-2 border">
              <div className='w-full flex justify-between items-center gap-2'>
                <div className='flex items-center justify-end gap-4'>
                  <span
                    className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center text-white font-bold text-lg uppercase">{(user.firstName).charAt(0)}</span>
                  <span className='overflow-hidden'>{user.firstName}</span>
                </div>
          ...
          <DropdownMenuContent align='end'>
            {isLogged === true ? (
              ...
              <Button
                variant="ghost"
                className="w-full gap-2 text-green-500 hover:bg-green-500 hover:text-white"
